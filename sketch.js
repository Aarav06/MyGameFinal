...
function draw() {
  ...
  if(gameState === 7) {
    ...
    fill("white");
    textSize(20);
    text('How many gems are there?', 440, 300);
    submit.show();
    inputCount.show();
    submit.position = 550, 500;
  ...
  if(gameState == 11) {
    ...
    text('Press space to start and to reset the ball', 170, 600);
    text('Destroy the Blocks!', 600, 100);
  ...
  background(lerpColor(startColor, newColor, amt));
    ...
    fill("white");
    textSize(40);
    text('Well done '+ playerName +'! You have completed my game!', 180, 300);
    text('You are one of ' +playerCount +' to do so!', 320 , 400);
  }
}
...
if(keyCode === 32){
 ...
 sling.attach(ball.body);
  ...
  //getCount from database
  function getCount(){
    var gameCount  = database.ref('count');
